...
from keras.layers import Flatten # maggie
TIME_STEP = 2
THRESHOLD = 0.5
...
class Lstm(Algorithm):
    ...
    def learning(self,features,dataset,label,kind, epochs=50, patience=None):
        ...
        print(f"{self.get_name()} {kind} dataset_min:{dataset_min}")
        print(f"{self.get_name()} {kind} dataset_max:{dataset_max}")
        #----------------------
        fallback = False
        ...
        #-----------maggie
        # self.classifier[kind].save(f'/home/huan1932/IoTDataGenerate/data_preprocess/result/model/vanilla_{self.get_name()}_{kind}_detector_trained_without_normalization.h5')
        # self.classifier[kind].save(f'/home/huan1932/IoTDataGenerate/data_preprocess/result/model/vanilla_{self.get_name()}_{kind}_detector_trained_with_default_normalization.h5')
    ...
    def detection(self, dataset, label, kind):
        ...
        #---------maggie-------
        self.testset_min = np.min(test)
        self.testset_max = np.max(test)
        print(f"{self.get_name()} {kind} testset_min:{self.testset_min}")
        print(f"{self.get_name()} {kind} testset_max:{self.testset_max}")
        #----------------------
