...
The rationale behind this modular design is as follows.
First, the detectors specialized in different steps are more effective as they have higher accuracy than a single monolithic detector that detects all the events of the steps altogether~\cite{modular}.
Second, \sys mainly focuses on preventing infections by identifying infection patterns and improving the infection classifier. The modular approach allows for an efficient \textit{evolution} of the system as whenever an anomaly at a step is detected, \sys can upgrade the infection detector individually without affecting the other detectors.
Third, such modular design helps with explainability for attacks. 
% A single monolithic detector with a large neural network is a blackbox, 
With the detection results, our modular architecture can provide information about the attack step (\eg Reconnaissance, Infection, or Action) that the adversary is performing, which is helpful to understand the attack in detail.
% our modular architecture may provide information about what step of the attack is coming into the system. 
Finally, in terms of security, \sys is more resilient to attempts aiming at evading detection as the attacks would need to avoid detection by all the detectors.

Note that a window may belong to multiple steps. For example, the window can be classified as \textit{Reconnaissance} and \textit{Infection} by the reconnaissance and the infection detectors. We call such a window both a reconnaissance window and an infection window. As the results of per-step detectors can be false positives, we make our per-step detectors return confidence scores as well as the results. \sys applies the softmax function to normalize the confidence scores from per-step detectors, resulting in a probability distribution. The probability distribution is used to correct false positives by the infection identification algorithm. We call an output of per-step detectors an \textit{event} that contains a window, three labels (\ie whether the window belongs to each step respectively), and four probabilities (\ie normalized confidence scores). 
